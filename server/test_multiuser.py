...
def test_health_check():
    ...
    try:
        ...
        print("❌ Cannot connect to server. Make sure it's running on http://localhost:8081")
        return False
...
def get_all_memories(user_id, limit=100):
    ...
    response = requests.get(f"{BASE_URL}/memories", params={"user_id": user_id, "limit": limit})
    return response
...
def test_multi_user_isolation():
    ...
    # Alice's memories
    alice_memory1 = add_memory(alice_id, "I love Italian food, especially pizza", {"category": "food"})
    alice_memory2 = add_memory(alice_id, "I work as a software engineer", {"category": "work"})
    
    # Bob's memories  
    bob_memory1 = add_memory(bob_id, "I prefer Japanese cuisine like sushi", {"category": "food"})
    bob_memory2 = add_memory(bob_id, "I'm a data scientist", {"category": "work"})
    
    # Charlie's memories
    charlie_memory1 = add_memory(charlie_id, "Mexican food is my favorite", {"category": "food"})
    charlie_memory2 = add_memory(charlie_id, "I'm a product manager", {"category": "work"})
    
    # Check that all memories were added successfully
    if not all([r.status_code == 200 for r in [alice_memory1, alice_memory2, bob_memory1, bob_memory2, charlie_memory1, charlie_memory2]]):
    ...
    if alice_memories.status_code != 200 or bob_memories.status_code != 200 or charlie_memories.status_code != 200:
    ...
    if alice_food_search.status_code != 200 or bob_food_search.status_code != 200 or charlie_food_search.status_code != 200:
    ...
    alice_has_pizza = any("pizza" in result.get("content", "").lower() for result in alice_food_results)
    bob_has_sushi = any("sushi" in result.get("content", "").lower() for result in bob_food_results)
    charlie_has_mexican = any("mexican" in result.get("content", "").lower() for result in charlie_food_results)
    ...
    if alice_results:
        ...
        cross_access_attempt = requests.get(f"{BASE_URL}/memories/{alice_memory_id}", params={"user_id": bob_id})
        
        if cross_access_attempt.status_code == 404 or cross_access_attempt.status_code == 403:
    ...
    print(f"   After Alice's delete - Alice: {alice_count_after}, Bob: {bob_count_after}, Charlie: {charlie_count_after}")
