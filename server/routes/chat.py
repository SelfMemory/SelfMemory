...
from server.chat.tools import AVAILABLE_TOOLS, SearchMemoriesParams, AddMemoryParams
from server.chat.memory_client import search_memories, add_memory
...
async def stream_chat(
    ...
    if not session_cookie_value:
        print(f"[Chat] No session cookie found. Available cookies: {list(cookies.keys())}")
...
Always search memories first when questions reference past information. Provide clear, helpful answers based on the retrieved memories."""
    ...
    if enable_tools:
        ...
        print(f"[Chat] Tool calling enabled with system prompt")
    else:
        print(f"[Chat] Tool calling disabled - set ENABLE_TOOL_CALLING=true to enable")
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        print(f"[Chat] vLLM error: {response.status_code} - {error_text}")
                                        ...
                                        for tc_delta in delta["tool_calls"]:
                                            ...
                                            if index not in tool_call_accumulator:
                                                tool_call_accumulator[index] = {
                                                    "id": tc_delta.get("id", f"call_stream_{index}"),
                                                ...
                                                print(f"[Tool Call Stream] Started accumulating tool call #{index}")
                                            ...
                                            if "function" in tc_delta:
                                                ...
                                                if "arguments" in tc_delta["function"]:
                                                    ...
                                                    print(f"[Tool Call Stream] #{index} accumulated args: {tool_call_accumulator[index]['function']['arguments'][:100]}...")
                    ...
                    if tool_call_accumulator:
                        for index in sorted(tool_call_accumulator.keys()):
                            ...
                            print(f"[Tool Call Stream] Finalized #{index}: {accumulated_tc['function']['name']} with {len(accumulated_tc['function']['arguments'])} chars of args")
                ...
                if xml_tool_calls:
                    print(f"[Chat] Parsed {len(xml_tool_calls)} XML tool call(s) from Qwen3")
                    has_tool_calls = True
                ...
                if has_tool_calls:
                    ...
                    for tool_call in tool_calls:
                        ...
                        if "function" in tool_call:
                            ...
                            function_args_str = json.dumps(tool_call.get("arguments", {}))
                            tool_call_id = tool_call.get("id", "call_unknown")
                        
                        try:
                            function_args = json.loads(function_args_str) if isinstance(function_args_str, str) else function_args_str
                            print(f"[Tool Call] ✓ Successfully parsed arguments for {function_name}")
                        except json.JSONDecodeError as e:
                            print(f"[Tool Call] ✗ Failed to parse arguments: {e}")
                            print(f"[Tool Call] Raw arguments string: '{function_args_str[:200]}'")
                            function_args = {}
                        
                        print(f"[Tool Call] Executing {function_name} with args: {function_args}")
                        ...
                        if function_name == "search_memories":
                            ...
                            print(f"[Tool Call - Memory Search] ===== QUERY DEBUG =====")
                            print(f"[Tool Call - Memory Search] Raw function_args: {function_args}")
                            print(f"[Tool Call - Memory Search] Extracted query: '{search_query}'")
                            print(f"[Tool Call - Memory Search] Query length: {len(search_query)}")
                            print(f"[Tool Call - Memory Search] Limit: {search_limit}")
                            print(f"[Tool Call - Memory Search] Project ID: {chat_request.projectId}")
                            
                            # If query is empty, use a default that retrieves recent memories
                            if not search_query:
                                print(f"[Tool Call - Memory Search] ⚠️ Empty query detected, using fallback '*' for recent memories")
                                search_query = "*"
                            else:
                                print(f"[Tool Call - Memory Search] ✓ Valid query: '{search_query}'")
                            
                            # Execute memory search
                            print(f"[Tool Call - Memory Search] 🔍 Calling search_memories with query='{search_query}'")
                            ...
                            print(f"[Tool Call - Memory Search] 📊 Retrieved {len(memories)} memories")
                            for i, mem in enumerate(memories[:3]):  # Log first 3 memories
                                print(f"[Tool Call - Memory Search] Memory #{i+1}: score={mem.score:.4f}, content='{mem.content[:100]}...'")
                            ...
                            if memory:
                                ...
                                print(f"[Tool Result] Failed to add memory")
                    ...
                    if not validated_tool_calls:
                        print(f"[Tool Call] No valid tool calls after validation, skipping second request")
                        ...
                        messages_with_tools = (
                            ...
                            [{"role": "assistant", "content": full_response or "", "tool_calls": validated_tool_calls}] +
                            tool_messages
                        )
                        ...
                        try:
                            async with client.stream(
                                ...
                                if final_response.status_code != 200:
                                    ...
                                    print(f"[Chat] Second request error: {final_response.status_code} - {error_text}")
        ...
        try:
            ...
            if isinstance(arguments, dict):
                ...
                try:
                    ...
                    print(f"[XML Parser] Invalid JSON string in arguments, using empty object")
                    arguments_str = "{}"
            else:
                print(f"[XML Parser] Invalid arguments type, using empty object")
                arguments_str = "{}"
            ...
            print(f"[XML Parser] ✓ Parsed tool call: {tool_data.get('name')} with args {arguments_str[:100]}")
            ...
            try:
                ...
                if name_match:
                    ...
                    print(f"[XML Parser] Salvaging tool call with name: {function_name}")
                    tool_call = {
                        "id": f"call_qwen3_{i}",
                        "type": "function",
                        "function": {
                    ...
                    print(f"[XML Parser] Could not salvage tool call")
