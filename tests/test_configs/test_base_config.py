...
Tests the InMemoryConfig and related configuration classes following
 focused approach - testing what matters for our use case.
"""
...
class TestVectorStoreConfig:
    ...
    def test_qdrant_config_with_url_no_path_default(self):
        ...
        config = VectorStoreConfig(
            provider="qdrant", config={"url": "http://localhost:6333"}
        )
        defaults = config.get_config_with_defaults()
...
class TestEmbeddingConfig:
    ...
    def test_config_with_custom_values(self):
        ...
        config = EmbeddingConfig(
            provider="ollama", config={"model": "custom-model", "embedding_dims": 1024}
        )
        defaults = config.get_config_with_defaults()
...
#
# This follows  approach of testing what actually matters:
