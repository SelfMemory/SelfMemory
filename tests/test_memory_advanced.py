"""
Advanced Memory class tests inspired by  patterns.
...
class TestMemoryParametrized:
    ...
    def test_memory_init_different_providers(
        self, provider_combo, mock_ollama_embedding, mock_qdrant_vector_store
    ):
        """
        TEST EXPLANATION:
        ================
        ...
        - Follows  parametrized testing pattern
    ...
    @pytest.mark.parametrize(
        "memory_content,expected_success",
        [
    ...
    def test_add_memory_content_validation(
        self,
        ...
        if not memory_content.strip():
            ...
            mock_memory_instance.embedding_provider.embed.side_effect = ValueError(
    ...
    def test_search_with_different_limits(
        ...
        mock_results = [
            create_mock_point(id=f"test-{i}", content=f"Memory {i}")
...
class TestMemoryAdvancedScenarios:
    """Advanced test scenarios inspired by  comprehensive testing."""

    def test_memory_with_dict_config(
        self, mock_ollama_embedding, mock_qdrant_vector_store
    ):
        """
        TEST EXPLANATION:
        ================
        What I'm testing: Memory initialization with dictionary config ( pattern)

        Advanced scenario:
        - Tests the dict-to-config conversion functionality
        - Ensures our API supports both dict and object configs
        - Follows  flexible config approach
    ...
    def test_concurrent_memory_operations(
        ...
        assert (
            mock_memory_instance.embedding_provider.embed.call_count == 3
        )  # 2 adds + 1 search
    ...
    def test_memory_with_complex_metadata(
        ...
        complex_metadata = {
            "user_info": {
                ...
                "settings": {"notifications": True, "privacy_level": "medium"},
            },
            "context": {
                "session_id": "sess_456",
    ...
    def test_search_with_advanced_filters(
        self, mock_memory_instance, mock_embedding_response, create_mock_point
    ):
        """
        TEST EXPLANATION:
...
class TestMemoryErrorHandling:
    """Comprehensive error handling tests inspired by  robustness."""
    ...
    def test_memory_resilience_to_provider_failures(self, mock_memory_instance):
        ...
        mock_memory_instance.embedding_provider.embed.side_effect = ConnectionError(
        ...
        mock_memory_instance.vector_store.insert.side_effect = TimeoutError(
            "Vector store timeout"
        )

        result = mock_memory_instance.add(
            memory_content="Test during vector store failure"
        )
        assert result["success"] is False
