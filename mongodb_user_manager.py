...
class MongoUserManager:
    ...
    def validate_api_key(self, api_key: str) -> Optional[str]:
        ...
        try:
            ...
            api_record = self.api_keys.find_one({
                "api_key": api_key,
                "is_active": True
            })
...
mongo_user_manager: Optional[MongoUserManager] = None
...
def get_mongo_user_manager() -> MongoUserManager:
    ...
    if mongo_user_manager is None:
        raise RuntimeError("MongoDB user manager not initialized. Call initialize_mongo_user_manager() first.")
    return mongo_user_manager
