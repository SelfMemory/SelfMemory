...
class MongoDBStore(MemoryStoreInterface):
    ...
    def migrate_from_file_store(self, file_store: "FileBasedStore") -> bool:
        """
        Migrate data from a FileBasedStore to MongoDB.

        Args:
            file_store: Source FileBasedStore instance
        ...
        try:
            ...
            users_data = file_store._read_json(file_store.users_file)
            api_keys_data = file_store._read_json(file_store.api_keys_file)
            ...
            for user_id, user_info in users_data.items():
                if self.create_user(user_id, **user_info):
                    migrated_users += 1
            ...
            for key_id, key_info in api_keys_data.items():
                ...
                if user_id and api_key:
                    ...
                    metadata = {
                        ...
                        if k not in ["user_id", "api_key"]
                    }

                    if self.store_api_key(user_id, api_key, **metadata):
                        migrated_keys += 1

            logger.info(
                f"Migration completed: {migrated_users} users, {migrated_keys} API keys"
            )
            return True
