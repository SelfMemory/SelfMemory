...
from constants import DuplicateConstants, VectorConstants, MetadataConstants, SearchConstants
...
class DuplicateDetector:
    ...
    def __init__(self, similarity_threshold: float = None):
        ...
        """
        self.similarity_threshold = similarity_threshold or DuplicateConstants.SIMILARITY_THRESHOLD
        
    def check_for_duplicates(self, memory_content: str, metadata: Dict[str, Any] = None) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        ...
        try:
            logger.info(f"Checking for duplicates of: '{memory_content[:50]}...'")
            ...
            logger.info(f"Duplicate check result: {is_duplicate}, found {len(similar_memories)} similar memories")
    ...
    def _find_similar_memories(self, query_vector: List[float], memory_content: str, metadata: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
                ...
                if MetadataConstants.MEMORY_FIELD in point.payload:
                    memory_data = {
                        ...
                        'metadata': {
                            ...
                            'people_mentioned': point.payload.get(MetadataConstants.PEOPLE_FIELD, []),
...
class DuplicateHandler:
    ...
    def handle_duplicate(
        ...
        try:
            if action == DuplicateConstants.DUPLICATE_ACTION_SKIP:
                ...
                return DuplicateHandler._merge_duplicate(new_memory, existing_memories, new_metadata)
    ...
    @staticmethod
    def _merge_duplicate(new_memory: str, existing_memories: List[Dict[str, Any]], new_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        merged_content = f"{most_similar['memory']}\n\n[Additional context: {new_memory}]"
    ...
    @staticmethod
    def _add_anyway(new_memory: str, existing_memories: List[Dict[str, Any]]) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    try:
        is_duplicate, similar_memories = detector.check_for_duplicates(test_memory, test_metadata)
        print(f"Is duplicate: {is_duplicate}")
        print(f"Similar memories found: {len(similar_memories)}")
        
        for memory in similar_memories[:3]:  # Show top 3
            print(f"  - Score: {memory['score']:.3f}, Content: {memory['memory'][:50]}...")
