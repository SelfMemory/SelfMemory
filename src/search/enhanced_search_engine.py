...
from constants import (
    VectorConstants, MetadataConstants, SearchConstants, 
    TemporalConstants, SearchFilters
)
...
class EnhancedSearchEngine:
    ...
    def search_memories(
        ...
        if not (SearchConstants.MIN_SEARCH_LIMIT <= limit <= SearchConstants.MAX_SEARCH_LIMIT):
            raise ValueError(f"Limit must be between {SearchConstants.MIN_SEARCH_LIMIT} and {SearchConstants.MAX_SEARCH_LIMIT}")
        
        try:
            logger.info(f"Enhanced search for: '{query[:50]}...' with filters")
    ...
    def temporal_search(
        ...
        """
        try:
            logger.info(f"Temporal search: '{temporal_query}' + semantic: '{semantic_query}'")
            
            # Parse temporal query
            temporal_filters = self.temporal_processor.parse_temporal_query(temporal_query)
            ...
            filter_conditions = self.temporal_filter.build_temporal_conditions(temporal_filters)
            ...
            if semantic_query:
                ...
                search_result = client.scroll(
                    collection_name=VectorConstants.COLLECTION_NAME,
                    scroll_filter=Filter(**filter_conditions) if filter_conditions else None,
                    limit=limit,
                    with_payload=True
    ...
    def _build_comprehensive_filter(
        ...
        try:
            ...
            if temporal_filter:
                temporal_filters = self.temporal_processor.parse_temporal_query(temporal_filter)
                if temporal_filters:
                    temporal_conditions = self.temporal_filter.build_temporal_conditions(temporal_filters)
    ...
    def _format_search_results(self, search_points: List) -> List[Dict[str, Any]]:
        ...
        for point in search_points:
            if not hasattr(point, 'payload') or MetadataConstants.MEMORY_FIELD not in point.payload:
                continue
            ...
            result = {
                ...
                'temporal_data': point.payload.get(MetadataConstants.TEMPORAL_FIELD, {})
            }
