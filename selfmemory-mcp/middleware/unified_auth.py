...
from auth.unified_auth import detect_and_validate_auth, TokenContext
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            token_context: TokenContext = await detect_and_validate_auth(
                auth_header,
                self.core_server_host
            )
            
            # Validate project context exists (required for tools)
            if not token_context.project_id:
                logger.error(f"❌ Token missing project context: user={token_context.user_id}")
